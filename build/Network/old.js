...
var attachCleanup = function (self, peer, socket) {
    socket.on("close", function () {
        ...
        if (peer.socket)
            return;
        if (!peer.host)
            return delete self.peers[peer.id];
        var reconnect = function () {
            connect(self, peer);
        };
        peer.retries++;
        // peer.reconnectTimeout = setTimeout(reconnect, (1 << peer.retries) * 250);
        peer.reconnectTimeout = setTimeout(reconnect, 1000);
...
var connect = function (self, peer, socket) {
    ...
    if (!socket)
        socket = net.connect(peer.port, peer.host);
...
var Topology = function (me, peers) {
    if (!(this instanceof Topology))
        return new Topology(me, peers);
    if (/^\d+$/.test(me))
        me = networkAddress() + ":" + me;
    this.me = me || "";
    this.peers = {};
    this.server = null;
    if (this.me)
        this.listen(Number(me.split(":")[1]));
    events.EventEmitter.call(this);
    if (peers)
        [].concat(peers).forEach(this.add.bind(this));
};
